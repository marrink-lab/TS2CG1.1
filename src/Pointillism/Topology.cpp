...
void Topology::FastReadQFile(std::string Filename)
{
    ...
    // read the box size from the q file.
    input>>Lx>>Ly>>Lz;
    m_Box(0)=Lx;
    m_Box(1)=Ly;
    m_Box(2)=Lz;
    ...
    for (int i=0;i<nover;i++)
    {
        ...
        if(Line.size()==5)
        {
            ...
            domain = f.String_to_Int(Line.at(4));
        }
        else
        {
            molid = 0;
    ...
    input>>notri;// number of triangles
    ...
    for (int i=0;i<notri;i++)
    {
        ...
        if(Line.size()<5)
        {
            std::cout<<"Error: Something wrong with the triangle list at triangle "<<i<<"\n";
            m_TopologyHealth = false;
            break;
        }
        id=f.String_to_Int(Line.at(0));
    ...
    for (std::vector<links>::iterator it = m_Links.begin() ; it != m_Links.end(); ++it)
    {
        ...
        if(foundM == false)
        {
            std::string A=" Warning (error for this version): This system does not look like a closed system or the triangles orination are not consistent ";
            std::cout<<A<<"\n";
            m_TopologyHealth = false;
            break;
        }
    ...
    // checking if there is any stand alone vertex: this type of vertices will have zero area,
    for (std::vector<vertex*>::iterator it = m_pAllV.begin() ; it != m_pAllV.end(); ++it)
    {
        if(((*it)->GetVLinkList()).size()==0)
        {
            std::cout<<" error: vertex with id "<<(*it)->GetVID()<<" does not have any connecting link, stand alone vertex, remove it \n";
            m_TopologyHealth = false;
            break;
        }
        if(((*it)->GetVTraingleList()).size()==0)
        {
            std::cout<<" error: vertex with id "<<(*it)->GetVID()<<" does not have any nighbouring triangle, stand alone vertex, remove it \n";
            m_TopologyHealth = false;
            break;
        }
        if(((*it)->GetVNeighbourVertex()).size()==0)
        {
            std::cout<<" error: vertex with id "<<(*it)->GetVID()<<" does not have any nighbouring vertex, stand alone vertex, remove it \n";
            m_TopologyHealth = false;
            break;
        }
    ...
    /*  double CM;
     for (std::vector<vertex>::iterator it = m_Vertex.begin() ; it != m_Vertex.end(); ++it)
     {
     CM = CM + (*it).GetVZPos()/double(m_Vertex.size());
     }
     for (std::vector<vertex>::iterator it = m_Vertex.begin() ; it != m_Vertex.end(); ++it)
     {
     ...
     Write(m_pAllV, m_pAllT,  "domain.q");*/
}
void Topology::Write(std::vector< vertex* > ver, std::vector< triangle* > triangle1,  std::string Filename)
{
    ...
    // incase the user did not provide the full file name
    ...
    // first write the box size, and then vertices
    output<<std::fixed;
    ...
    {
    output<<std::setprecision( pres )<<j<<"  "<<(*it)->GetVXPos()<<" "<<(*it)->GetVYPos()<<" "<<(*it)->GetVZPos()<<" "<<(*it)->GetDomainID()<<"\n";
    j++;
    }
    // write the triangles
