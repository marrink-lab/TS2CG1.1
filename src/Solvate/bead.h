...
public:
	...
	bead(int id, std::string name, std::string type, std::string resname, int resid);
	 ~bead();
        ...
        inline std::string GetResName()                 {return m_ResName;}
        inline int GetResid()         const        {return m_Resid;}
        inline std::string GetBeadName()                {return m_BeadName;}
...
public:
  ...
  void UpdateBox(Vec3D* z);
    void UpdateBeadName(std::string );
    void UpdateResName(std::string );
