...
Nfunction f;      // In this class there are some useful function and we can use it.
    ...
    int Nx = int((*m_pBox)(0)/BL)+1;
    int Nz = int((*m_pBox)(2)/BL)+1;
  ...
  //   if(step==0 && count>0)
   //      break;
    ...
    for (int i=0;i<Nx;i++)
    {
        for (int j=0;j<Nz;j++)
        {
            TargetMat(i,j) = TargetMat(i,j)/double(acceptedframe);
    ...
    output<<"   i     x      "<<targetname<<"updensity   "<<targetname<<"downdensity   "<<referencename<<"updensity   "<<referencename<<"downdensity   "<<"\n";
    for (int i=0;i<Nx;i++)
    {
        output<<i<<"   "<<BLx*i<<"   "<<TagetVec(i,1)<<"   "<<TagetVec(i,2)<<"    "<<RefVec(i,1)<<"   "<<RefVec(i,2)<<"  "<<BLx*TagetVec(i,0)<<"  "<<BLx*RefVec(i,0)<<"\n";
    }
